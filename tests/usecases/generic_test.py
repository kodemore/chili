...
def test_encode_generic_type() -> None:
    ...
    array_of_books = Array[Book](
        count=2,
        items=[Book("Book 1", 100), Book("Book 2", 200)]
    )

    array_of_authors = Array[Author](
        count=2,
        items=[Author("John", "Doe"), Author("Jane", "Doe")]
    )
    ...
    # then
    assert books == {"count": 2, "items": [{"name": "Book 1", "pages": 100}, {"name": "Book 2", "pages": 200}]}
    assert authors == {"count": 2, "items": [{"first_name": "John", "last_name": "Doe"}, {"first_name": "Jane", "last_name": "Doe"}]}
